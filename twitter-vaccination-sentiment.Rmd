...
In this document, we will provide methods on how to identify Twitter users and tweets that are "anti-vaccination."

<center>
![](data/word_cloud1.png)
</center>
...
The learning objectives for this case study include data cleaning with regular expressions, data exploration and visualization using Natural Language Processing techniques, sentiment analysis, and topic modeling.
...
```
Next we use `ggplot2` to plot this on a map.
```{r}
...
There are other things we could do get more lattitude and longitude coordinates. We could convert state names (for example, "Maryland") to their abbreviations ("MD"). We could also ensure the first letter of the city is capitalized and the rest of the letters are lowercase. These additional manipulations are left as an exercise.
...
tweet_sentiment <- text_df %>%
  ...
  group_by(int) %>%
  mutate(text = paste0(word)) %>%
  summarize(sentiment_mean = mean(value))
...
# Appendix
Here we illustrate how to generate unique IDs for each user and reconstruct the dataframe of tweets with this information. This is only necessary if you want to post the data and assumptions about the data. Please read the Twitter developer policies found [here](https://developer.twitter.com/en/developer-terms/agreement-and-policy.html).

Let `d` be the data we created and exported in the "What is the data?" section. First, use the function `dplR::uuid.gen` to generate unique IDs.
```{r eval = FALSE}
unique_ids <- unique(c(d$user_id, d$retweet_user_id, d$quoted_user_id))

library(dplR)
ug <- uuid.gen()
uuids <- character(length(unique_ids))
for(i in 1:length(unique_ids)){
  uuids[i] <- ug()
}
```
We create a data frame `id_merge_df` with two columns: `unique_ids` has the unique user ids from users that created the tweets and those the tweets were retweeted from. The second column `uuidgens_user` contains a list of unique ids. We consider each row as an assignment of a unique id to a unique user.

```{r eval = FALSE}
id_merge_df <- data.frame("unique_id" = unique_ids, "uuidgens_user" = uuids)
head(id_merge_df)

#Merge on the users
df_withIDs <- merge(d, id_merge_df, by.x = "user_id", by.y = "unique_id", all.x = TRUE, all.y = FALSE, sort = FALSE)

# Rename column to merge on the retweeted users
colnames(id_merge_df)[2]<-"uuidgens_retweet"
df_withIDs <- merge(df_withIDs, id_merge_df, by.x = "retweet_user_id", by.y = "unique_id", all.x = TRUE, all.y = FALSE, sort = FALSE)

# Rename columnd to merge on the quoted users
colnames(id_merge_df)[2]<-"uuidgens_quote"
df_withIDs <- merge(df_withIDs, id_merge_df, by.x = "quoted_user_id", by.y = "unique_id", all.x = TRUE, all.y = FALSE, sort = FALSE)

head(df_withIDs[c("user_id","uuidgens_user", "retweet_user_id", "uuidgens_retweet", "quoted_user_id","uuidgens_quote")])
```
Set the uuids that are associated with "na" to na.
```{r eval = FALSE}
na_uuid <- id_merge_df$uuidgens_quote[is.na(id_merge_df$unique_id)]
print(na_uuid)
df_withIDs$uuidgens_retweet[df_withIDs$uuidgens_retweet == na_uuid]<-NA
df_withIDs$uuidgens_quote[df_withIDs$uuidgens_quote == na_uuid]<-NA
```
Next, save to a csv:
```{r eval=FALSE}
write.csv(df_withIDs, file = "data_name.csv", row.names = FALSE, fileEncoding = "UTF-8")
```
