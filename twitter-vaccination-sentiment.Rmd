...
First let's create a matrix/dataframe called $S$ ("S" for social network) where $S_{ij} =$ the number of interactions (retweets) between user $i$ and user $j$.
```{r}
#unique_ids <- unique(c(tweets_df$uuidgens_user, tweets_df$uuidgens_retweet))
#S <- data.frame(matrix(data = 0, nrow = length(unique_ids), ncol = length(unique_ids)))

#colnames(S) <- unique_ids
#head(S)[1:3]
#dim(S)
#dim(tweets_df)
```
Let's first get the information we need from `tweets_df` and sort by the order of the unique ids.
```{r}
new_df <- tweets_df %>%
...
new_df <- new_df[
  order( new_df$uuidgens_user, new_df$uuidgens_retweet ),
  ] 

rownames(new_df) <- NULL
colnames(new_df)[1] <- "user"
colnames(new_df)[2] <- "rt"
head(new_df)
length(new_df$user)

unique_ids <- unique(c(new_df$user, new_df$rt))
```
Now we have condensed the data we need to loop through in order to create this matrix.
```{r}
S <- data.frame(matrix(data = 0, nrow = length(unique_ids), ncol = length(unique_ids)))
colnames(S) <- unique_ids


index <- c(1:length(unique_ids))


for (i in unique_ids[1:5]) {
  user_lst <- which(new_df$user == i)
  "printing list of retweets"
  print(user_lst)
  
  for (j in user_lst){
    rt_id <- new_df$rt[j]
    rt_index <- index[unique_ids == rt_id]
    print("printing original value")
    print(S[[i]][rt_index])
    "printing value + 1"
    S[[i]][rt_index] = S[[i]][rt_index] + 1
  }
  print(S[[i]][rt_index])

}
head(new_df)
print(sum(rowSums(S, na.rm=TRUE)))

T<- data.frame(matrix(data = 0, nrow = 5, ncol = 5))
T
for (i in colnames(T)){
  for (j in c(1:5)){
    T[[i]][j] <- T[[i]][j] + 1
