...
```{r}
pat <- "[\r\n]|&amp.|@.*?\\S+|@.*?$|https?:.*\\S+|https?:.*$|[^[:alnum:][:blank:]?&/\\-]"#U000.*?$`|U000.*?\\S+`"
...
```{r}
graph_df <- matrix(ncol=2, nrow=5000000)
index = 1
...
dim(graph_df)
# 
# reverse <- graph_df[c(2,1)]
# colnames(reverse)<-c("V1","V2")
# head(graph_df[c(1,2)])
# head(reverse)
# 
# final_combos <- cbind(graph_df[c(1,2)],reverse) %>%
#  group_by(V1,V2) %>%
#  tally()
